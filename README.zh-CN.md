...
> **当前版本**：`3.3.x.20241020-SNAPSHOT`<br>
> **JDK 基线**：`17`<br>
...
**pac4j-spring-boot-starter** 是一个面向 使用 pac4j 的应用 的 Spring Boot Starter，用于将 **pac4j** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 pac4j 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 pac4j 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 pac4j Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:pac4j-spring-boot-starter:3.3.x.20241020-SNAPSHOT` |
| 配置前缀 | `pac4j` |
...
| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 pac4j 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `pac4j.*` 到 `Pac4jCasProperties` |
| `Clients` Bean | ✅ 稳定 | 通过 Pac4jAutoConfiguration 自动注册 |
...
| Bean | 条件 | 缺失时行为 |
|---|---|---|
| `Clients` | classpath + property | 不创建 |
| `Config` | classpath + property | 不创建 |
...
<dependency>
    ...
    <version>3.3.x.20241020-SNAPSHOT</version>
</dependency>
```
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `pac4j.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `pac4j.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/pac4j-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/pac4j-spring-boot-starter)
